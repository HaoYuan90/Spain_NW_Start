...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		ClearNWImpact = AssignStartingPlots.ClearNWImpact,
...
------------------------------------------------------------------------------
-- Start of custom functions tied to BalanceAndAssign()
------------------------------------------------------------------------------
function AssignStartingPlots:ClearNWImpact(x, y, radius)
	-- This function cleans up ripple for natural wonder layer from a location x,y
	...
	if radius > 0 and radius < iH / 2 then
			...
			for direction_index = 1, 6 do
				for plot_to_handle = 1, ripple_radius do
					...
					elseif wrapY == false and (nextY < 0 or nextY >= iH) then -- Y is out of bounds.
						...
						-- Clear ripple data for this plot.
						local ringPlotIndex = realY * iW + realX + 1;
						self.naturalWondersData[ringPlotIndex] = 0;
					end
					currentX, currentY = nextX, nextY;
				end
			end
		...
		print("Unsupported Radius length of ", radius, " passed to ClearNWImpact()");
	end
end
------------------------------------------------------------------------------
function AssignStartingPlots:BalanceAndAssign()
	...
	-- TODO : Add special logic to handle spain here
	local humanSpainFound = false;
	for loop = 1, self.iNumCivs do
		local playerNum = self.player_ID_list[loop]; -- MP games can have gaps between player numbers, so we cannot assume a sequential set of IDs.
		local player = Players[playerNum];
		local civType = GameInfo.Civilizations[player:GetCivilizationType()].Type;
		if civType == "CIVILIZATION_SPAIN" and player:IsHuman() then
			print("[HYLOG]Human Spain player found.");
			humanSpainFound = true;
			local plot = player:GetStartingPlot();
			local x = plot:GetX();
			local y = plot:GetY();
			print("[HYLOG]Clearing NW ripples from X : ", x, "; Y : ", y);
			self:ClearNWImpact(x, y, 4)
			print("[HYLOG]Cleared NW ripples.");
		end
	end
	
	if humanSpainFound == false then
		print("[HYLOG]Human Spain player not found.");
	end
